...
class PathPlanner(Node):
    def __init__(self):
        ...
        self.waypoint_run_timer = self.create_timer(200000, self.move_to_waypoint, callback_group=callback_group_main) 
        
        self.waypoint_publisher = self.create_publisher(Waypoint, "desired_waypoint", 10) # msg type, topic_name to publish to, buffer size


        self.pose_subscriber = self.create_subscription(Pose, "estimated_pose", self.pose_callback, 10, callback_group= callback_group_main) 
